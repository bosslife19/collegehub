  ...
  return (
    ...
    {subscribe.map((items, index)=>(
        <div key={index} className='flex w-full  bg-[#91447B] justify-center border px-[25px] pt-[13px] pb-[0.3%] rounded-[25px] flex-col '>
           ...
           {items.subs.length > 0 && (
             <ul className="pb-[10px] md:pb-[20px] gap-4 flex flex-col pt-[30px] md:pt-[25%] ">
             {items.subs.map((sub, linkIndex) => (
               <li key={linkIndex} className=' flex gap-[10px] font-[500] leading-[23.7px] text-[14px] md:text-[18px]'>
                  <Goods/>
                   {sub.para1}
                   {sub.para2}
                   {sub.para3}
